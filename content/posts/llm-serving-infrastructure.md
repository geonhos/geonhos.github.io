...
- Apple Silicon vs NVIDIA 아키텍처 차이
- 하드웨어별 성능 비교
- 클라우드 LLM 서빙 인프라
...
### 메모리 타입 비교 — 왜 NVIDIA가 더 빠른가

NVIDIA가 빠른 이유는 "분리되어서"가 아니라 **대역폭 최적화된 전용 메모리**를 쓰기 때문입니다.

| 메모리 타입 | 버스 폭 | 대역폭 | 설계 목적 |
|:---|---:|---:|:---|
| <abbr data-tip="Low-Power DDR5, 모바일/저전력 환경에 최적화된 메모리">LPDDR5</abbr> (M1 Pro) | 256-bit | 200 GB/s | 저전력 효율 |
| <abbr data-tip="Low-Power DDR5X, LPDDR5의 고속 개선 버전">LPDDR5X</abbr> (M4 Max) | 512-bit | 546 GB/s | 저전력 + 빠르게 |
| <abbr data-tip="Graphics Double Data Rate 6X, 고성능 그래픽 전용 메모리">GDDR6X</abbr> (RTX 4090) | 384-bit | 1,008 GB/s | 최대 처리량 |
| <abbr data-tip="Graphics Double Data Rate 7, GDDR6X의 차세대 고속 메모리">GDDR7</abbr> (RTX 5090) | 512-bit | 1,792 GB/s | 차세대 처리량 |
| <abbr data-tip="High Bandwidth Memory 3, 수천 비트 버스 폭으로 극한 대역폭을 제공하는 메모리">HBM3</abbr> (H100) | 5,120-bit | 3,350 GB/s | 극한 대역폭 |
| <abbr data-tip="HBM3의 개선 버전, 더 높은 대역폭과 용량을 제공">HBM3e</abbr> (H200) | 5,120-bit | 4,800 GB/s | 차세대 극한 대역폭 |
...
아래 표의 하드웨어 스펙은 **각 제조사 공식 페이지에서 확인한 수치**입니다.

| 하드웨어 | VRAM | 대역폭 | 메모리 타입 | 출처 |
|:---|:---|---:|:---|:---|
| Apple M1 Pro | 통합 최대 32GB | 200 GB/s | LPDDR5 | [Apple Newsroom](https://www.apple.com/newsroom/2021/10/introducing-m1-pro-and-m1-max-the-most-powerful-chips-apple-has-ever-built/) |
| Apple M4 Max (40코어 GPU) | 통합 최대 128GB | 546 GB/s | LPDDR5X | [Apple 공식](https://support.apple.com/en-us/121553) |
| NVIDIA RTX 4090 | 24GB | 1,008 GB/s | GDDR6X, 384-bit | [NVIDIA 공식](https://www.nvidia.com/en-us/geforce/graphics-cards/40-series/rtx-4090/) |
| NVIDIA RTX 5090 | 32GB | 1,792 GB/s | GDDR7, 512-bit | [NVIDIA 공식](https://www.nvidia.com/en-us/geforce/graphics-cards/50-series/rtx-5090/) |
| NVIDIA H100 SXM5 | 80GB | 3,350 GB/s | HBM3 | [NVIDIA 공식](https://www.nvidia.com/en-us/data-center/h100/) |
| NVIDIA H200 | 141GB | 4,800 GB/s | HBM3e | [NVIDIA 공식](https://www.nvidia.com/en-us/data-center/h200/) |
...
H100 한 장의 VRAM이 80GB인데, 810GB 모델을 어떻게 올릴까요?

### 모델 병렬화 — GPU 여러 장에 쪼개서 올린다
...
> Llama 405B (810GB) → H100 16장 x 50GB = 800GB, NVLink 4.0 (900GB/s)으로 연결, 모델 전체 적재

이걸 **모델 병렬화**(Model Parallelism)라고 합니다.
...
```mermaid
flowchart TB
    subgraph DGX["DGX H100 Node — 8 x H100 = 640GB VRAM"]
        direction LR
        H1["H100<br/>80GB"] <-->|NVLink| H2["H100<br/>80GB"] <-->|NVLink| H3["H100<br/>80GB"] <-->|NVLink| H4["H100<br/>80GB"]
        H5["H100<br/>80GB"] <-->|NVLink| H6["H100<br/>80GB"] <-->|NVLink| H7["H100<br/>80GB"] <-->|NVLink| H8["H100<br/>80GB"]
    end
```

> GPU간 통신: NVLink 900GB/s | 노드 가격: ~4억원 | x 수백~수천 노드 = 수천 장의 GPU

GPU간 통신 속도가 중요한 이유: 텐서 병렬화에서 매 레이어마다 GPU끼리 중간 결과를 교환해야 합니다. PCIe(~31.5GB/s)로는 병목이 심각하지만, NVLink(900GB/s)는 PCIe의 **28배** 대역폭을 제공합니다.

### Continuous Batching — 수백만 명이 동시에 쓴다

```mermaid
flowchart LR
    U1["User 1: 맛집 추천"] --> CB["Continuous<br/>Batching"]
    U2["User 2: 코드 리뷰"] --> CB
    U3["User 3: 번역"] --> CB
    U4["User 1000: 요약"] --> CB
    CB --> GPU["GPU Cluster"]
...
| 클라우드 서빙 | 모델 병렬화 + Continuous Batching + NVLink |
